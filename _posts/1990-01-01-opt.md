...
- TPBM = TPBN = 32.  This achieves the max threads per block (1024); dividing it evenly between rows and columns minimizes the total amount of data that needs to be loaded into shared memory.
- TM = TN = 8.  I'm not sure what the best value is here (we'll come back to this below) but let's start with 8.
- BK = 16.  This is dictated by the 48KB limit on shared memory.  (Actually, I could make this value as large as 24, but maybe it's better to stick with powers of 2 to start.)
